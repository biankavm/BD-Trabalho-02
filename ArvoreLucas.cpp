...
No* ler_registro_do_bloco_index(char* bloco, int tam_bloco) {
    No *no = inicializaNo(tam_bloco);
    no->chaves[0] = -1;
    no->enderecos[0] = -1;
    ...
    for (int reg = 0; reg < pos; reg++) {
        for (int i = 0; i < size; i++) {
            id_str[i] = bloco[reg+i];
            printf("%d ",id_str[i]);
        }
        id_atual = converter_chars_para_int(id_str);
        printf("(%s , %d) ",id_str,id_atual);

        no->chaves[reg]=id_atual;
        //printf("leu chave\n");
    }

    for (int reg = pos; reg < tam_bloco; reg++) {
        for (int i = 0; i < size; i++) {
            id_str[i] = bloco[reg+i];
        }
        id_atual = converter_chars_para_int(id_str);

        no->enderecos[reg-pos]=id_atual;
        //printf("leu end\n");
    }
    printf("aaaaaaaaa\n %d, %d\n",no->chaves[0],no->enderecos[0]);
    return no;
...
int insercao(int chave, int enderecoRegistro, int tamBloco) {
    ...
    while (n->chaves[q-1] == 0) { //enquanto não é uma folha, pois o ultimo valor do vetor de chaves indica se é ou não uma folha
        S.push(n); // guarda o papai
        Send.push(1); //provavelmente o endereço da raiz vai ser 1
        if (chave <= n->chaves[0]) {
            n = carregaNo(n->enderecos[0]); 
            Send.push(n->enderecos[0]);
        }
        else if (chave > n->chaves[q - 2]) {
            n = carregaNo(n->enderecos[q - 2]);
    ...
    if(folhaCheia == 1) //se a folha não está cheia, aí complica
    {
        ...
        int *chaveTemp = criarVetorAleatorio(q);
        int *endTemp = criarVetorAleatorio(q);
        int y = 0;
        chave = temp->chaves[j];
